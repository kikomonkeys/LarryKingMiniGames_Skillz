...
public class WinnerControllerScript : MonoBehaviour {
    ...
    void Start() {
        ...
        if (PoolGame_GameManager.Instance.playerDisconnected) {
            PoolGame_GameManager.Instance.playerDisconnected = false;
            if (!isGameScene) {
        ...
        if (!isGameScene) {
            ...
            if (PoolGame_GameManager.Instance.payoutCoins > PoolGame_GameManager.Instance.coinsCount) {
                rematchButton.SetActive(false);
            }
            ...
            if (PoolGame_GameManager.Instance.avatarMy != null)
                myImage.sprite = PoolGame_GameManager.Instance.avatarMy;
            if (PoolGame_GameManager.Instance.avatarOpponent != null)
                oppoImage.sprite = PoolGame_GameManager.Instance.avatarOpponent;

            myName.text = PoolGame_GameManager.Instance.nameMy;
            oppoText.text = PoolGame_GameManager.Instance.nameOpponent;
        ...
        for (int i = 0; i < StaticStrings.chatMessagesExtended.Length; i++) {
            if (PoolGame_GameManager.Instance.ownedChats.Contains("'" + i + "'")) {
    ...
    public void loadMenuScene() {
       ...
       // if (!GameManager.Instance.offlineMode)
        ...
        PoolGame_GameManager.Instance.cueController.removeOnEventCall();
