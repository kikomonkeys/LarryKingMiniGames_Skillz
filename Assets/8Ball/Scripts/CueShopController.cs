...
public class CueShopController : MonoBehaviour {
    ...
    void Start() {
        ...
        if (PoolGame_GameManager.Instance.ownedCues.Contains("'" + index + "'")) {
            button.GetComponent<Image>().sprite = blueSprite;
            buttonText.GetComponent<Text>().text = "Use";
            own = true;
        }

        if (PoolGame_GameManager.Instance.cueIndex == index) {
            PoolGame_GameManager.Instance.usingCueText = buttonText;
    ...
    public void buyCue() {
        if (own) {
            ...
            PoolGame_GameManager.Instance.usingCueText.GetComponent<Text>().text = "Use";
            PoolGame_GameManager.Instance.usingCueText = buttonText;
            buttonText.GetComponent<Text>().text = "Using";
